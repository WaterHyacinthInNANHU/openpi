#!/usr/bin/env bash
# Launch v2 training (pi05_droid_franka_lora_5k) on all 8 labserver GPUs.
# Runs detached (invoke with: setsid bash ... > /tmp/train_v2.log 2>&1 < /dev/null &)
set -u
...
exec env CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
  "$UVPY" scripts/train.py pi05_droid_franka_lora_5k \
  --exp-name=pi05_droid_franka_lora_5k_v0 --overwrite
