...
# Layout: <openpi>/examples/tasl_franka/eval/launch/lib.sh
TASL="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"            # the eval stack (this dir's parent)
OPENPI_DIR="${OPENPI_DIR:-$(cd "$TASL/../../.." && pwd)}"          # the openpi checkout: serve_policy + openpi-client
DESKTOP_HOME="$(getent passwd "${SUDO_USER:-$USER}" | cut -d: -f6)"  # operator home even under sudo
DESKTOP_HOME="${DESKTOP_HOME:-$HOME}"
REPO="${RLINF_REPO_HOST:-$DESKTOP_HOME/RLinf}"                     # RLinf checkout (collection stack + rlinf-eval container mount)
TASL_DATA_DIR="${TASL_DATA_DIR:-$DESKTOP_HOME/tasl_data}"          # eval_episodes/ saved_demo/ logs/ (outside the repo)
LOG_DIR="$TASL_DATA_DIR/logs"; mkdir -p "$LOG_DIR" 2>/dev/null || true
SITE_PKGS="$DESKTOP_HOME/.local/lib/python3.10/site-packages"
export OPENPI_DIR TASL_DATA_DIR
...
# zed_serials: the serials the ZED SDK can currently enumerate, as a string.
# Probes inside the rlinf container when it is up (that is where collect/teleop
# open the cameras) and falls back to the HOST interpreter otherwise — the
# openpi dashboard opens the ZEDs on the host anyway, and both see the same
# devices (the container is --privileged with -v /dev:/dev).
#
# The container fallback is not cosmetic: `docker exec` against a stopped
# container prints "container ... is not running" on stderr and nothing on
# stdout, which the old code swallowed with 2>/dev/null. An exited container
# was therefore indistinguishable from "both cameras unplugged" — it reported
# ZEDs not SDK-visible, USB-reset two healthy cameras, and told the operator to
# reseat cables and reboot. Costs an afternoon; ask how we know.
zed_serials() {
  local py='import pyzed.sl as sl; print([d.serial_number for d in sl.Camera.get_device_list()])'
  if [[ "$(docker inspect -f '{{.State.Running}}' "$RLINF_CONTAINER" 2>/dev/null)" == "true" ]]; then
    docker exec "$RLINF_CONTAINER" /opt/venv/openpi/bin/python -c "$py" 2>/dev/null | tail -1
  else
    /usr/bin/python3 -c "$py" 2>/dev/null | tail -1
  fi
}

# zeds_missing: which expected serials the SDK cannot see, space-separated
# ("" = all present). ALLOW_MISSING_WRIST=1 waives the wrist ZED-M (degraded
# mode — pair with the openpi dashboard's --allow-missing-wrist flag).
zeds_missing() {
  local serials; serials=$(zed_serials)
  local want=("$ZED_EXTERIOR") missing=() sn
  [[ "${ALLOW_MISSING_WRIST:-}" == 1 ]] || want+=("$ZED_WRIST")
  for sn in "${want[@]}"; do
    [[ "$serials" == *"$sn"* ]] || missing+=("$sn")
  done
  echo "${missing[*]}"
}
...
zeds_free_or_reset() {
  ...
  for ((i = 1; i <= tries; i++)); do
    warn "ZEDs not SDK-visible (missing: $(zed_names "$(zeds_missing)")) — auto USB-reset (attempt $i/$tries)"
    zeds_reset
...
# zed_names / zed_usb_id: serial -> human label and USB id, so failures can name
# the camera the operator has to walk over and touch.
zed_names() {
  local sn out=()
  for sn in $1; do
    case "$sn" in
      "$ZED_EXTERIOR") out+=("$sn exterior ZED 2i") ;;
      "$ZED_WRIST")    out+=("$sn wrist ZED-M") ;;
      *)               out+=("$sn") ;;
    esac
  done
  local IFS=", "; echo "${out[*]}"
...
zed_usb_id() {
  case "$1" in
    "$ZED_EXTERIOR") echo 2b03:f880 ;;
    "$ZED_WRIST")    echo 2b03:f682 ;;
  esac
}

# zed_off_bus: of the given serials, those whose CAMERA USB interface is not on
# the bus at all. A missing serial that IS on the bus is an SDK wedge (usbreset
# clears it); one that is NOT is unplugged / a dead USB3 link / a bad cable, and
# no amount of usbreset will bring it back — only a physical reseat will. The
# ZED-M keeps its f681 HID interface alive on USB2 when its f682 USB3 video side
# drops, so the camera still "looks plugged in" in lsusb: check the video id.
zed_off_bus() {
  local sn id out=()
  for sn in $1; do
    id="$(zed_usb_id "$sn")"
    [[ -n "$id" ]] && ! lsusb 2>/dev/null | grep -qi "$id" && out+=("$sn")
  done
  echo "${out[*]}"
}
...
ensure_zeds_free() {
  ...
  # (c) the camera is not on the USB bus at all — reaping and usbreset are both
  # pointless, so say so instead of burning two reset attempts and then blaming
  # the SDK. Checked first because it is the one cause software cannot fix.
  local off; off="$(zed_off_bus "$(zeds_missing)")"
  if [[ -n "$off" ]]; then
    warn "NOT on the USB bus: $(zed_names "$off") — its camera interface is gone"
    return 1
  fi
  local held
...
preflight_robot() {
  ...
  local missing off
  missing="$(zeds_missing)"
  off="$(zed_off_bus "$missing")"
  ensure_zeds_free || {
    if [[ -n "$off" ]]; then
      # Software cannot fix this one — name the camera and the cable.
      die "ZED NOT ON THE USB BUS: $(zed_names "$off")
    Its camera interface ($(for s in $off; do zed_usb_id "$s"; done | tr '\n' ' ')) is absent from lsusb, so the SDK
    cannot see it and usbreset has nothing to reset. This is a physical link
    fault — a half-seated or failing USB3 cable, or a dead camera.
      1. reseat BOTH ends of that camera's USB3 cable (camera + Desktop port)
      2. re-run this script — no reboot needed if the cable was the problem
      3. only if it stays absent: try another USB3 port / another cable
    NOTE: a ZED-M keeps its f681 HID interface on USB2 when the USB3 video side
    drops, so it still shows up in lsusb — that is NOT proof the camera is fine.
    To run WITHOUT the wrist camera (policy goes half-blind, black wrist view):
      ALLOW_MISSING_WRIST=1 $0"
    fi
    die "ZED cameras still not visible to the SDK after reaping
    in-container holders AND a USB reset (missing: $(zed_names "$missing")).
    They ARE on the USB bus, so this is an SDK/driver wedge. Escalate:
      1. $TASL/launch/zed-check.sh          # confirm which serial is missing
      2. physically unplug/replug that camera, then re-run
