...
packaged as an **opt-in module**: nothing in openpi's own scripts/ is modified, and
...
```bash
<openpi>/.venv/bin/python <openpi>/examples/tasl_franka/eval/rtc/scripts/serve_policy.py --port 8000 \
    policy:checkpoint --policy.config=pi05_droid_franka_lora \
    --policy.dir=$HOME/ckpts/pi05_droid_franka_lora_10task/16000 &
PYTHONPATH=$HOME/.local/lib/python3.10/site-packages:<openpi>/packages/openpi-client/src \
    /usr/bin/python3 <openpi>/examples/tasl_franka/eval/rtc/smoke.py --s 4 --d 3
```
...
JAX_PLATFORMS=cpu <openpi>/.venv/bin/python -m pytest tests/test_rtc_math.py tests/test_rtc_executor.py -q
