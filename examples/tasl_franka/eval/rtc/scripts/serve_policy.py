...
"""RTC-capable drop-in for openpi/scripts/serve_policy.py — identical CLI.
    ...
    <openpi>/.venv/bin/python <openpi>/examples/tasl_franka/eval/rtc/scripts/serve_policy.py \
...
serve_policy.py, default = this checkout) and wraps the policy in RTCPolicy, so
...
TASL_DIR = pathlib.Path(__file__).resolve().parents[2]          # <openpi>/examples/tasl_franka/eval
# This file ships inside the openpi checkout it serves from: <openpi>/examples/tasl_franka/eval/rtc/scripts/
OPENPI_DIR = pathlib.Path(os.environ.get("OPENPI_DIR") or TASL_DIR.parents[2])
