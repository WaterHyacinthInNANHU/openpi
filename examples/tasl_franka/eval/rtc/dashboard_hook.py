...
def run_episode(runner, prompt: str, policy) -> None:
    ...
    def infer(obs: dict, req: dict):
        ...
        if runner._iter >= runner.max_iterations:  # noqa: SLF001
            # Step cap == eval timeout. Flag it BEFORE _stop so EvalRunner's
            # finally can tell "timed out" from an operator Stop (both end up
            # with _stop set on this path) and auto-mark the episode fail.
            runner._cap_hit = True  # noqa: SLF001
