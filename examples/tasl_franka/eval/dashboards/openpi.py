...
def _real_home() -> str:
    """Home of the operator, not root's: the launcher re-execs under sudo (HOME=/root)."""
    import pwd
...
# Layout of this checkout: <openpi>/examples/tasl_franka/eval/dashboards/openpi.py
_TASL_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))          # .../eval
OPENPI_DIR = os.environ.get("OPENPI_DIR", os.path.abspath(os.path.join(_TASL_DIR, "..", "..", "..")))
_HOME_DIR = os.environ.get("TASL_HOME", _real_home())                              # /home/franka_desktop
# All mutable bench data lives OUTSIDE the repo. One root, sub-dirs below; each is overridable.
DATA_DIR = os.environ.get("TASL_DATA_DIR", os.path.join(_HOME_DIR, "tasl_data"))
_PATCH_DIR = os.environ.get("PI05_PATCH_DIR", os.path.join(DATA_DIR, "VLA-PatchLen-cp"))
...
DEMO_DIR = os.environ.get("RLINF_DEMO_DIR", os.path.join(DATA_DIR, "saved_demo"))
...
OPENPI_VENV_PY = os.environ.get(
    "OPENPI_VENV_PY", os.path.join(OPENPI_DIR, ".venv/bin/python"))
...
HOME_STORE_PATH = pathlib.Path(os.path.join(DATA_DIR, "_dashboard_home.json"))
...
class EvalRunner:
    ...
    def __init__(self, cam_mgr: CamManager, home_store: "HomeStore",
        ...
        # Episode step cap == the eval timeout. Hitting it ends the episode
        # AND auto-marks it fail (see _cap_hit / on_cap_reached below):
        # a policy that has not solved the task in max_iterations steps has
        # failed, so the operator should not have to press ✗ by hand.
        # Live-editable from the portal via /eval/max_iters.
        ...
        # feed back to model.
        self.audit_path = pathlib.Path(os.environ.get(
            "GRIP_AUDIT_PATH", os.path.join(os.path.expanduser("~"), ".tasl", "_grip_audit.jsonl")))
        ...
        self.homing = False
        # Step-cap (timeout) state. _cap_hit is set by whichever loop noticed
        # _iter reach max_iterations — the two paths exit differently (the
        # sync loop falls out of its while with _stop CLEAR, the RTC path
        # sets _stop itself), so an explicit flag is the only reliable
        # "timed out" signal. _loop's finally hands it to on_cap_reached,
        # wired by build_app to the same mark→export→home flow as /eval/mark.
        self._cap_hit = False
        self.on_cap_reached: Optional[Callable[[], None]] = None
    ...
    def watchdog_step(self, q: np.ndarray, cmd_mag: float,
        ...
        if len(self._wd_hist) >= self.WD_STEPS and cmd_mean > self.WD_CMD_MIN \
            ...
            # Read the liveness verdict BEFORE _wd_reset() — the reset clears
            # _wd_ts_armed, so testing it afterwards always saw False and the
            # "arm blocked, keep going" branch below was unreachable. That made
            # every contact task (lid closing, insertion) abort as if the NUC
            # were dead: 20 rule-(2) aborts and 0 "arm blocked" warnings in
            # logs/openpi.log, while rule (1) has been silent since the
            # 2026-08-26 P-core pinning fix and the NUC state timestamp
            # advances on every step (500/500 distinct in ep_20260830_174252).
            loop_alive = ts_ns is not None and self._wd_ts_armed
            self._wd_reset()
    ...
    def start(self, prompt: str, tag: Optional[dict] = None) -> str:
        with self._lock:
            ...
            self._recorder = EvalRecorder(
                ...
                image_mode=str(meta.get("image_mode") or ""),
    ...
    def halt_tick(self) -> None:
        """One zero-velocity tick so the arm stops at its current pose instead
        of coasting toward the last streamed setpoint. Keeps the last gripper
        command so halting mid-grasp doesn't drop the object. stop() does this
        via its _halt thread; the step-cap path (loop exits by itself) needs
        it standalone."""
        try:
            if self._droid is not None and self._droid.bootstrapped:
                a8 = [0.0] * 8
                if self._last_grip_raw is not None:
                    a8[7] = max(0.0, min(1.0, float(self._last_grip_raw)))
                self._droid.update_joint_velocity(a8, blocking=False)
                _log.info("halt tick sent (step cap)")
        except Exception as exc:  # noqa: BLE001
            _log.warning(f"halt tick failed: {exc}")
    ...
    def _loop(self, prompt: str):
        try:
            ...
            # Fell out of the while with _stop still clear and the counter at
            # the cap → the episode timed out (as opposed to Stop / Mark,
            # which set _stop, or a watchdog/error break, which sets
            # _last_error and leaves _iter short).
            if self._iter >= self.max_iterations and not self._stop.is_set():
                self._cap_hit = True
            ...
            self._running = False
            self._thread = None
            # Timed out → auto-mark. Must run AFTER _running=False so the
            # recorder (running_cb) finalizes and meta.json lands; the
            # callback waits for it. Own thread: it stops the arm, exports
            # video and streams home, which must not block this loop's exit.
            if self._cap_hit and self.on_cap_reached is not None:
                threading.Thread(target=self.on_cap_reached, daemon=True).start()
...
SERVE_CMD_DIR = OPENPI_DIR   # serve_policy.py runs from THIS checkout (same code as training)
...
# Per-user, not /tmp: with fs.protected_regular=2 a sticky-dir file is only writable by its owner
# (root included), so alternating root/user dashboard launches left a /tmp/serve_ckpt.log the other
# identity could not open ("spawn failed: Permission denied"). Override with SERVE_LOG.
_TASL_STATE_DIR = os.path.join(os.path.expanduser("~"), ".tasl")
os.makedirs(_TASL_STATE_DIR, exist_ok=True)
SERVE_LOG = os.environ.get("SERVE_LOG", os.path.join(_TASL_STATE_DIR, "serve_ckpt.log"))
...
# ─────────────────────────────────────────────────────────────────────
EPISODES_ROOT = os.environ.get("TASL_EVAL_EPISODES", os.path.join(DATA_DIR, "eval_episodes"))
...
class EvalRecorder:
    ...
    def __init__(self, cam_mgr, task_id: str, prompt: str, ckpt: str,
                 ...
                 abort_source=None, layout: str = "", image_mode: str = ""):
        self.cam_mgr = cam_mgr
        # Which policy-input preprocessing this rollout ran under: "pad"
        # (DROID letterbox) or "crop" (pbc center-crop). Recorded so a rollout
        # can never again be mistaken for the other arm's image geometry.
        self.image_mode = image_mode or ""
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* live step / cap counter, sits with the Mark buttons */
  .stepchip { display: inline-flex; align-items: center; gap: 5px;
    padding: 4px 9px; border: 1px solid var(--border); border-radius: 6px;
    background: #101010; font-size: 13px; color: var(--dim);
    font-variant-numeric: tabular-nums; }
  .stepchip input[type=number] { width: 62px; padding: 3px 5px; font-size: 12.5px; }
  .stepchip #stepCur { font-size: 15px; color: var(--text); min-width: 3.2ch;
    text-align: right; }
  .stepchip.run   #stepCur { color: var(--ok); }
  .stepchip.near  #stepCur { color: var(--warn); }
  .stepchip.near  { border-color: #5A4A32; }
  .stepchip.over  #stepCur { color: var(--err); }
  .stepchip.over  { border-color: #5A3232; }
...
<div class="card" id="taskCard">
  ...
  <div class="flexbar" style="margin-top:6px">
    ...
    <label style="margin-left:10px;white-space:nowrap"
           title="CFG 微调 ckpt(cotrain_cfg_v2)训练时每条 prompt 末尾都带 '\nQuality: N'(训练用 5);推理必须同样追加,否则走不到条件分支。PBC / DROID ckpt 不要勾。CFG ckpt 同样需要勾 pbc 裁剪。">
      <input type="checkbox" id="cfgOn" onchange="cfgUpdate()"/> CFG quality</label>
    <select id="cfgQ" onchange="cfgUpdate()" disabled style="width:54px" title="Quality 1..5(训练用 5)">
      <option>1</option><option>2</option><option>3</option><option>4</option><option selected>5</option>
    </select>
    <span id="cfgInfo" style="font-size:12px;color:var(--faint)"></span>
  </div>
  <div class="flexbar">
...
<div class="row" style="margin-top:16px;align-items:stretch">
  <div class="col card" style="margin-bottom:0">
    ...
    <div class="flexbar" style="margin-top:8px">
      ...
      <span class="stepchip" id="stepChip"
            title="当前 step / 上限。上限就是超时:跑满自动判 ✗ fail、导出 demo、回 home(和手按 Mark fail 完全一样)。上限可直接改,立刻生效——包括正在跑的 episode,所以能当场掐掉磨蹭的 rollout 或给还在推进的续命。">
        <b id="stepCur">0</b><span style="color:var(--faint)">/</span>
        <input id="maxIters" type="number" min="1" max="100000" step="50" value="1200"
               onchange="maxItersUpdate()"/>
        <span id="stepEta" style="color:var(--faint);font-size:12px"></span>
      </span>
    ...
    <div class="flexbar">
      <button id="btnSaveVideo" onclick="saveVideo()" disabled
              title="手动导出最新一条录制(Mark ✓/✗ 已经自动导出)— 视频(exterior|wrist)+meta+traj → saved_demo/<task>[-ood]/<ckpt>/<layout>_rNN_T|F|Q/">💾 Save demo</button>
    ...
    <div class="hint">Start 前先在上方选择任务;Mark ✓/✗ 写入最新一条 eval 录制的
      meta 并<b>自动导出</b>视频到 RLinf/saved_demo/&lt;task&gt;[-ood]/&lt;ckpt&gt;/&lt;layout&gt;_rNN_T|F|Q/(每个 rollout 一个文件夹;T=成功 F=失败 Q=Stop 自动记录、待定)
      (layout 名含 ood 时进 -ood 目录;NN = 该 layout 的第几条 rollout;Stop 不导出,
      aborted 不导出)。<b>超时</b>:step 跑满上限(Mark 按钮旁那个计数器)= 自动判 ✗ fail,
      同样导出 demo 并回 home,episode 表里记作 ✗⏱;若该条已被 watchdog 判为 abort(NUC 不执行),
      则记 aborted 不算 fail。上限可随时改,对正在跑的 episode 立即生效。
      Save demo 手动导出最新一条;Save png 导出当前选中
...
function cfgUpdate() {
  const on = document.getElementById('cfgOn').checked;
  const sel = document.getElementById('cfgQ');
  sel.disabled = !on;
  document.getElementById('cfgInfo').textContent =
    on ? '→ prompt 末尾追加 "\\nQuality: ' + sel.value + '"' : '';
  try { localStorage.setItem('cfgOn', on ? '1' : '0'); localStorage.setItem('cfgQ', sel.value); } catch (e) {}
}
function cfgQuality() {
  return document.getElementById('cfgOn').checked
    ? parseInt(document.getElementById('cfgQ').value) : null;
}
function pbcUpdate() {
...
try {
  ...
  if (localStorage.getItem('cfgQ')) document.getElementById('cfgQ').value = localStorage.getItem('cfgQ');
  if (localStorage.getItem('cfgOn') === '1') document.getElementById('cfgOn').checked = true;
  cfgUpdate();
} catch (e) {}
...
async function startEval() {
  ...
  const j = await api('/start', {
    ...
    cfg_quality: cfgQuality(),
  });
  if (j && j.ok !== false) {
    setSaveVideo(false);
    if (j.prompt) consoleLine('start: prompt sent to policy = ' + JSON.stringify(j.prompt), 'lvl-I');
  }
  return j;
}
...
async function stopEval() {
  ...
  if (j && j.ok !== false) {
    ...
    else if (j.demo) { si.textContent = '💾 ❓ ' + j.demo.msg + ' — 在 episode 表里点 ✓/✗ 定判'; }
    if (j.ep_id) epRefresh();
  }
  return j;
}
...
// ── step / 上限 实时计数(Mark 按钮旁)────────────────────────────────
// 上限 = 超时。跑满服务端自动走 ✗ fail 流程(见 /eval/max_iters、_on_step_cap)。
function stepChipUpdate(re) {
  const chip = document.getElementById('stepChip');
  const cur  = document.getElementById('stepCur');
  const inp  = document.getElementById('maxIters');
  const eta  = document.getElementById('stepEta');
  if (!chip || re.iter === undefined) return;
  const cap = re.max_iterations || Number(inp.value) || 0;
  cur.textContent = re.iter;
  // 别在用户正在输入时把值冲掉。
  if (document.activeElement !== inp && cap) inp.value = cap;
  // 剩余步数 → 大约还能跑多久(15 Hz 名义;实际含推理耗时会更久)。
  const left = Math.max(0, cap - re.iter);
  eta.textContent = re.running && cap ? '剩~' + Math.round(left / 15) + 's' : '';
  const frac = cap ? re.iter / cap : 0;
  chip.classList.toggle('run',  re.running && frac < 0.8);
  chip.classList.toggle('near', re.running && frac >= 0.8 && frac < 1);
  chip.classList.toggle('over', !!re.cap_hit || frac >= 1);
}

async function maxItersUpdate() {
  const inp = document.getElementById('maxIters');
  const n = parseInt(inp.value, 10);
  if (!Number.isFinite(n) || n < 1) { toast('上限必须是正整数', 'err'); return; }
  const j = await api('/eval/max_iters', {max_iterations: n});
  if (j && j.ok) toast('step 上限 → ' + j.max_iterations
                       + '(约 ' + Math.round(j.max_iterations / 15) + 's),跑满自动 ✗ fail', 'ok');
  else toast('设置失败: ' + ((j && j.msg) || ''), 'err');
}
...
async function refresh() {
  try {
    ...
    if (lastEvalRunning === true && re.running === false && lastEvalIter > 0) {
      ...
      if (re.cap_hit) {
        // 跑满上限 = 超时;服务端正在自动判 fail → 导出 demo → 回 home。
        consoleLine('timeout: 跑满 ' + (re.max_iterations || lastEvalIter)
                    + ' step 上限 → 自动 ✗ fail + 导出 + 回 home', 'lvl-E');
        toast('⏱ 超时 — 跑满 ' + (re.max_iterations || lastEvalIter)
              + ' step,自动判 ✗ fail、导出 demo、回 home', 'err');
        const el = document.getElementById('evalMarkInfo');
        if (el) el.textContent = '⏱ 超时 → 自动 ✗ fail,导出 + 回 home…';
        // home 流 + 导出要几秒,等落盘后再刷列表拿到最终 mark。
        setTimeout(epRefresh, 6000);
      } else {
...
function onTaskSelect() {
  ...
  // Keep a hand-picked layout across re-renders of the same task; only fall back to the
  // task's last-used layout (t.layout) when the task changed or the pick is no longer valid.
  const keep = (laySel.dataset.task === t.id && lays.indexOf(laySel.value) >= 0) ? laySel.value : '';
  taskGhostLayout = keep || (lays.length ? (t.layout || lays[lays.length - 1]) : '');
  laySel.dataset.task = t.id;
...
function epTally(eps) {
  const t = {ok: 0, fail: 0, unmarked: 0, unsure: 0};
  eps.forEach(e => { if (e.mark === 'success') t.ok++;
                     else if (e.mark === 'fail') t.fail++;
                     else if (e.mark === 'unsure') t.unsure++;
...
function epTallyHtml(t) {
  return '<b style="color:#81C995">✓' + t.ok + '</b> <b style="color:#F28B82">✗'
    + t.fail + '</b>'
    + (t.unsure ? ' <span style="color:#E5C07B">❓' + t.unsure + '</span>' : '')
      ...
      const ckLine = Object.keys(byCk).length > 1
        ? '<div class="byckpt">by ckpt: ' + Object.keys(byCk).sort().map(k => {
            ...
            return esc(k) + ' <b style="color:#81C995">' + c.ok + '</b>/'
              + c.n + ((c.unmarked + c.unsure) ? '<span style="color:#E5C07B">+' + (c.unmarked + c.unsure) + '</span>' : '')
              + (c.sr === null ? '' : ' (' + c.sr + '%)');
          }).join(' · ') + '</div>' : '';
      const rows = g.eps.map(e => {
        const mark = e.mark === 'success' ? '<b style="color:#81C995">✓</b>'
                   : e.mark === 'fail' ? (e.timeout
                       ? '<b style="color:#F28B82" title="超时自动判负 — 跑满 '
                         + (e.timeout_steps || '?') + ' step 上限">✗⏱</b>'
                       : '<b style="color:#F28B82">✗</b>')
                   : e.mark === 'unsure' ? '<b style="color:#E5C07B" title="Stop 自动记录,待人工判定">❓</b>'
        ...
        return '<tr>'
          ...
          + (e.demo && e.demo.stem ? '<br><span style="color:var(--faint)" title="saved_demo/' + esc(e.demo.dir.split('/').slice(-2).join('/')) + '">💾 ' + esc(e.demo.stem) + '</span>' : '')
          + '</td>'
          + '<td>' + esc(e.ckpt || '-')
          ...
          + '<button class="tiny" data-ep="' + esc(e.ep_id) + '" data-mark="fail" onclick="markEpRec(this.dataset.ep,this.dataset.mark)">✗</button>'
          + '<button class="tiny" data-ep="' + esc(e.ep_id) + '" data-mark="unsure" title="待定" onclick="markEpRec(this.dataset.ep,this.dataset.mark)">❓</button></td>'
          + '<td>' + esc(e.note || '') + '</td>'
...
async function markEpRec(epId, mark) {
  ...
  if (j.ok) { if (j.demo) document.getElementById('saveInfo').textContent = '💾 ' + j.demo.msg; epRefresh(); }
}
...
def build_app(rs: RS, cams: CamManager, runner: EvalRunner,
    ...
    # ── Demo exports → RLinf/saved_demo/<task>[-ood]/<ckpt>/<layout>_rNN_X/<same>.* ─
    # Naming: directory <task>-ood when the layout id contains "ood", else
    # <task>, then one folder per checkpoint (_demo_ckpt_name); stem = <layout id>_r<NN>_<T|F> (the task id when the episode
    ...
    # Rollouts are grouped by checkpoint: saved_demo/<task>[-ood]/<ckpt name>/.
    # Human names for the ckpt labels the serve reports; anything else falls
    # back to a slug of the label (":" "/" " " → "-", "+rtc" dropped).
    _DEMO_CKPT_NAMES = {
        "pi05_droid_franka_lora_10task_v2/15000": "pi05-droid-ft-15k",
    }

    def _demo_ckpt_name(ckpt: str) -> str:
        c = (ckpt or "").strip()
        base = c[:-len(" +rtc")].strip() if c.endswith("+rtc") else c
        if base in _DEMO_CKPT_NAMES:
            return _DEMO_CKPT_NAMES[base]
        slug = re.sub(r"[^A-Za-z0-9.]+", "-", base).strip("-")
        return slug or "unknown-ckpt"
    ...
    def _demo_target(m: dict):
        ...
        return pathlib.Path(DEMO_DIR) / sub / _demo_ckpt_name(m.get("ckpt") or ""), (lay or task)

    # unsure = recorded by Stop without a verdict → "_Q", to be decided later.
    _DEMO_MARK_SUFFIX = {"success": "_T", "fail": "_F", "unsure": "_Q"}

    def _demo_rollout_no(out_dir: pathlib.Path, base: str) -> int:
        pat = re.compile(rf"^{re.escape(base)}_r(\d+)(?:_[TFQ])?$")
        n = 0
        for j in list(out_dir.glob(f"{base}_r*/")) + list(out_dir.glob(f"{base}_r*.json")):
            mm = pat.match(j.stem if j.is_file() else j.name)
            if mm:
    ...
    def _demo_folder(out_dir: pathlib.Path, stem: str) -> pathlib.Path:
        """One folder per rollout: <ckpt dir>/<stem>/<stem>.{mp4,traj.jsonl,frames.json,json}."""
        return out_dir / stem

    def _demo_files(out_dir: pathlib.Path, stem: str):
        d = _demo_folder(out_dir, stem)
        base = d if d.is_dir() else out_dir      # pre-2026-08-28 flat exports
        return [base / f"{stem}{suf}" for _n, suf in _DEMO_SIDE] + [base / f"{stem}.json"]

    def _demo_rename(out_dir: pathlib.Path, old: str, new: str) -> None:
        """Rename one rollout (files + folder) old → new; flat layout gets foldered."""
        src = _demo_folder(out_dir, old)
        dst = _demo_folder(out_dir, new)
        dst.mkdir(parents=True, exist_ok=True)
        for f in _demo_files(out_dir, old):
            if f.exists():
                f.rename(dst / (new + f.name[len(old):]))
        if src.is_dir() and src != dst and not any(src.iterdir()):
            src.rmdir()
    ...
    def _export_episode(meta_f: pathlib.Path, m: dict) -> dict:
        ...
        _hand_to_owner(folder, recursive=False)
        _hand_to_owner(*_demo_files(out_dir, stem), recursive=False)
        return {"dir": str(out_dir), "stem": stem, "files": saved,
                "path": str(folder / f"{stem}.mp4"),
                "msg": f"saved → {out_dir.name}/{stem}/{stem}.mp4 (+{len(saved) - 1} sidecars)"}
    ...
    def episode_update():
        ...
        meta_f, meta = hit
        if body.get("mark") not in (None, "", "success", "fail", "unsure", "aborted"):
            return jsonify({"ok": False, "msg": "mark must be success|fail|unsure|aborted"}), 400
        old_mark = meta.get("mark") or ""
        ...
        new_mark = meta.get("mark") or ""
        if new_mark in _DEMO_MARK_SUFFIX and (new_mark != old_mark or not demo.get("stem")):
            # Verdict changed (or first verdict on a not-yet-exported episode):
            # (re)export so the _T/_F/_Q suffix follows the mark.
            try:
                r = _export_episode(meta_f, meta)
                return jsonify({"ok": True, "msg": f"updated · {r['msg']}", "demo": r})
            except Exception as e:  # noqa: BLE001
                _log.warning(f"episode update: demo export failed: {e}")
                return jsonify({"ok": True, "msg": f"updated · demo export failed: {e}"})
        if demo.get("dir") and demo.get("stem"):
            jf = _demo_files(pathlib.Path(demo["dir"]), demo["stem"])[-1]
            if jf.exists():
    ...
    def episode_delete():
        ...
        if demo.get("dir") and demo.get("stem"):
            ...
            fd = _demo_folder(pathlib.Path(demo["dir"]), demo["stem"])
            if fd.is_dir() and not any(fd.iterdir()):
                fd.rmdir()
    ...
    def demo_compact():
        ...
        pat = re.compile(r"^(?P<base>.+)_r(?P<n>\d+)(?P<suf>_[TFQ])?$")
        renamed = []
        dirs = []
        for sub in (task, f"{task}-ood"):
            d = pathlib.Path(DEMO_DIR) / sub
            if d.is_dir():
                dirs.append(d)                       # legacy flat exports
                dirs.extend(sorted(c for c in d.iterdir() if c.is_dir()))  # <ckpt>/
        for d in dirs:
            sub = str(d.relative_to(DEMO_DIR))
            groups: dict = {}
            seen = set()
            for jf in list(d.glob("*/")) + list(d.glob("*.json")):
                name = jf.name if jf.is_dir() else jf.stem
                if jf.is_file() and (jf.name.endswith(".frames.json") or jf.name.startswith("layout_")):
                    continue
                mm = pat.match(name)
                if mm and name not in seen:
                    seen.add(name)
                    groups.setdefault(mm.group("base"), []).append(
                        (int(mm.group("n")), name, mm.group("suf") or ""))
            for base, items in groups.items():
                ...
                if not plan:
                    continue
                # Two-phase so a→b, b→a chains never collide.
                for old, new in plan:
                    _demo_rename(d, old, f"__compact__{new}")
                for old, new in plan:
                    _demo_rename(d, f"__compact__{new}", new)
                for old, new in plan:
                    jf = _demo_files(d, new)[-1]
                    try:
    ...
    def post_start():
        ...
        # CFG quality tag (front-end "CFG quality" checkbox + 1..5 picker). The CFG fine-tune
        # (pi05_cotrain_franka_lora_10task_cfg_v2) was trained with every prompt ending in
        # "\nQuality: N" (openpi.training.quality_conditioning on the WaterHyacinthInNANHU/openpi
        # tasl-franka-cfg branch; constant 5 at train time), so inference must append the same
        # suffix or the conditional branch is never hit. Appended HERE, before runner.start(), so
        # the policy loop, the status line and the recorder's meta.json all carry the exact
        # string the model saw. Unchecked -> plain prompt (PBC / DROID checkpoints).
        cfg_q = body.get("cfg_quality")
        if cfg_q not in (None, "", False):
            try:
                cfg_q = int(cfg_q)
            except (TypeError, ValueError):
                return jsonify({"ok": False, "msg": f"bad cfg_quality {cfg_q!r}"}), 400
            if not 1 <= cfg_q <= 5:
                return jsonify({"ok": False, "msg": f"cfg_quality must be 1..5, got {cfg_q}"}), 400
            prompt = f"{prompt}\nQuality: {cfg_q}"
            _log.info(f"CFG quality tag {cfg_q} -> prompt={prompt!r}")
        ...
        # Guard: the policy loop connects to :8000 with openpi's WebsocketClientPolicy, which
        # retries forever. Starting with no serve loaded leaves the thread stuck at iter 0 where
        # Stop cannot reach it and every checkpoint switch is refused ("eval running, stop first").
        st = serve.status() or {}
        if st.get("state") != "ready":
            return jsonify({"ok": False,
                            "msg": f"no policy server ready (serve state={st.get('state')!r}) -- "
                                   "load a checkpoint first"}), 409
        msg = runner.start(prompt, tag=tag)
        return jsonify({"ok": True, "msg": msg, "prompt": prompt, "tag": tag})

    def _wait_meta(rec, timeout: float = 15.0):
        """meta.json path of the recorder once its loop has flushed it (None
        when there is no recorder or it did not land in time)."""
        if rec is None:
            return None
        meta_f = rec.out_dir / "meta.json"
        deadline = time.time() + timeout
        while not meta_f.exists() and time.time() < deadline:
            time.sleep(0.2)
        return meta_f if meta_f.exists() else None

    @app.post("/stop")
    def post_stop():
        """Stop the rollout. A running episode is kept as a rollout too: it is
        marked "unsure" (❓, not counted in the SR) and exported as
        <layout>_rNN_Q so the operator can decide ✓/✗ later from the episode
        table. Watchdog-aborted episodes get "aborted" as with Mark."""
        rec = runner._recorder  # noqa: SLF001
        was_running = runner.status()["running"]
        msg = runner.stop()
        if not was_running:
            return jsonify({"ok": True, "msg": msg})
        meta_f = _wait_meta(rec)
        if meta_f is None:
            return jsonify({"ok": True, "msg": msg + " · recorder did not flush meta.json"})
        demo = None
        try:
            meta = json.loads(meta_f.read_text(encoding="utf-8"))
            if not meta.get("mark"):
                meta["mark"] = "aborted" if meta.get("abort") else "unsure"
                meta_f.write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
            if meta["mark"] == "unsure":
                demo = _export_episode(meta_f, meta)
                _log.info(f"stop: demo saved (unsure): {demo['dir']}/{demo['stem']}")
        except Exception as e:  # noqa: BLE001
            demo = {"error": str(e)}
            _log.warning(f"stop: auto-record failed: {e}")
        return jsonify({"ok": True, "msg": msg, "ep_id": rec.ep_id,
                        "mark": meta.get("mark") if demo is not None and "error" not in demo else None,
                        "demo": demo})

    # Serializes the verdict flow. Without it the step-cap auto-mark and an
    # operator pressing ✓/✗ at the same moment both stop → mark → export →
    # home, racing on meta.json and interleaving two home streams on the NUC.
    _mark_lock = threading.Lock()

    def _peek_mark(rec) -> str:
        """The mark already on this recorder's episode, or "" if the episode
        has no meta.json yet / is unmarked / is unreadable."""
        try:
            if rec is None:
                return ""
            f = rec.out_dir / "meta.json"
            if not f.exists():
                return ""
            return str(json.loads(f.read_text(encoding="utf-8")).get("mark") or "")
        except Exception:  # noqa: BLE001 — a peek must never break the flow
            return ""

    def _mark_and_home(mark: str, force: bool = False, timeout_hit: bool = False):
        """Stop the rollout, wait for the recorder to flush meta.json, write
        the mark, export the demo, then stream the arm home. Shared by
        /eval/mark (operator ✓/✗) and the step-cap timeout auto-fail.
        ...
        with _mark_lock:
            ...
            if was_running:
                ...
                # Cheap peek before touching the arm: if the operator's ✓/✗
                # already landed on this episode they have halted, exported
                # and homed it, so the timeout has nothing left to do and
                # must not fire a stray tick at an arm sitting at home.
                # (meta.json usually does NOT exist yet here — the recorder
                # is still finalizing — so the common path falls straight
                # through to the halt.)
                if _peek_mark(rec):
                    return ({"ok": True, "ep_id": getattr(rec, "ep_id", None),
                             "skipped": "already marked", "demo": None,
                             "home": "", "timeout": True}, 200)
                # The loop exited on its own at the cap, so nothing sent the
                # zero-velocity halt that stop() would have — without it the
                # arm coasts toward the last streamed setpoint while we wait
                # on meta.json.
                runner.halt_tick()
            ...
            if meta_f is not None:
                ...
                if not meta_f.exists():
                    return ({"ok": False, "ep_id": rec.ep_id,
                ...
                if hit is None:
                    return ({"ok": False, "msg": "no eval episode to mark"}, 404)
                meta_f = hit
            try:
                meta = json.loads(meta_f.read_text(encoding="utf-8"))
                if timeout_hit and meta.get("mark"):
                    # The operator got their verdict in first (pressed ✓/✗ in
                    # the seconds between the cap and this callback) — their
                    # call wins, and it already exported + homed.
                    return ({"ok": True, "ep_id": meta.get("ep_id") or meta_f.parent.name,
                             "mark": meta["mark"], "skipped": "already marked",
                             "demo": None, "home": ""}, 200)
                if meta.get("abort") and not force:
                    ...
                    # filter on those). {"force": true} overrides. A timed-out
                    # episode on a dead NUC is an abort, not a policy failure.
                    mark = "aborted"
                meta["mark"] = mark
                if timeout_hit:
                    # Distinguishes "policy ran out of steps" from an operator ✗
                    # in the episode table and any downstream analysis.
                    meta["timeout"] = True
                    meta["timeout_steps"] = runner.max_iterations
                meta_f.write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
            except Exception as e:
                return ({"ok": False, "msg": f"mark write failed: {e}"}, 500)
            ep_id = meta.get("ep_id") or meta_f.parent.name
            src = f"timeout@{runner.max_iterations} steps" if timeout_hit else "operator"
            _log.info(f"eval mark ({src}): {meta.get('task')}/{ep_id} → {mark}")
            ...
            # Let the halt tick land before commanding the home move.
            if was_running or timeout_hit:
                time.sleep(0.7)
            home_msg = "homed"
            runner.homing = True
            try:
                ...
                runner.homing = False
            return ({"ok": True, "ep_id": ep_id, "task": meta.get("task"),
                     "mark": mark, "stopped": was_running, "home": home_msg,
                     "timeout": timeout_hit, "demo": demo}, 200)

    def _on_step_cap():
        """Episode hit max_iterations → it timed out. A policy that has not
        finished in that many steps has failed, so run the exact ✗ flow:
        mark fail, export the demo, send the arm home. Called from the policy
        loop's finally on its own thread (see EvalRunner.on_cap_reached)."""
        try:
            payload, _ = _mark_and_home("fail", timeout_hit=True)
            if not payload.get("ok"):
                _log.error(f"step-cap auto-fail failed: {payload.get('msg')}")
        except Exception as e:  # noqa: BLE001 — never kill the callback thread
            _log.exception(f"step-cap auto-fail crashed: {e}")

    runner.on_cap_reached = _on_step_cap
    ...
    def eval_mark():
        ...
        payload, code = _mark_and_home(mark, force=bool(body.get("force")))
        return jsonify(payload), code

    @app.post("/eval/max_iters")
    def eval_max_iters():
        """Set the episode step cap (== the eval timeout). Takes effect on the
        next Start; changing it mid-episode also moves the running episode's
        cap, which is deliberate — it lets the operator cut a stuck rollout
        short, or extend one that is still making progress."""
        body = request.get_json(silent=True) or {}
        try:
            n = int(body.get("max_iterations"))
        except (TypeError, ValueError):
            return jsonify({"ok": False, "msg": "max_iterations must be an int"}), 400
        if not (1 <= n <= 100000):
            return jsonify({"ok": False, "msg": "max_iterations out of range (1..100000)"}), 400
        runner.max_iterations = n
        _log.info(f"max_iterations set to {n} (~{n / runner.control_hz:.0f}s of motion)")
        return jsonify({"ok": True, "max_iterations": n})
