...
# Shared with the collection dashboard (RLinf tasl/dashboards/collect.py): both must point at the
# same file. Default lives with the other shared bench data under ~/rlinf_data.
def _real_home() -> str:
    import pwd
...
_HOME = os.environ.get("TASL_HOME", _real_home())
TASKS_PATH = os.environ.get("TASL_TASKS_STORE", os.path.join(_HOME, "rlinf_data", "tasks_store.json"))
DATASET_ROOTS = [
    os.path.join(_HOME, "lerobot_home"),
    os.path.join(_HOME, "rlinf_data", "datasets"),
]
...
class TaskStore:
    ...
    def remove_layout(self, tid: str, layout_id: str) -> str:
        """Detach layout `layout_id` from task `tid` (the layout files are
        the caller's business). If it was the armed one, the newest
        remaining layout takes over."""
        layout_id = (layout_id or "").strip()
        if not layout_id:
            return "layout id required"
        with self._lock:
            ...
            for t in self.tasks:
                if t["id"] == tid:
                    lays = [l for l in (t.get("layouts") or []) if l != layout_id]
                    if len(lays) == len(t.get("layouts") or []) and t.get("layout") != layout_id:
                        return f"layout {layout_id} not on task {tid}"
                    t["layouts"] = lays
                    if t.get("layout") == layout_id:
                        t["layout"] = lays[-1] if lays else ""
                    t["updated"] = time.strftime("%Y-%m-%d %H:%M")
                    self._save()
                    return "removed"
        return f"task {tid} not found"

    def rename_layout(self, layout_id: str, new_id: str) -> list:
        """Replace `layout_id` with `new_id` in EVERY task that references it.
        ...
        with self._lock:
            ...
            for t in self.tasks:
                lays = t.get("layouts") or []
                hit = layout_id in lays or t.get("layout") == layout_id
                if not hit:
                    continue
                t["layouts"] = [new_id if l == layout_id else l for l in lays]
                if t.get("layout") == layout_id:
                    t["layout"] = new_id
                t["updated"] = time.strftime("%Y-%m-%d %H:%M")
                touched.append(t["id"])
    ...
    def layout_users(self, layout_id: str) -> list:
        """Ids of every task that still references `layout_id`."""
        with self._lock:
            self._refresh()
            return [t["id"] for t in self.tasks
                    if layout_id in (t.get("layouts") or []) or t.get("layout") == layout_id]
