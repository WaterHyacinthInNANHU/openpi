...
class LayoutStore:
    ...
    def rename(self, layout_id: str, new_id: str) -> str:
        """Rename `<id>.json` + its snapshots to `<new_id>.*` and rewrite the
        json's `id`. Callers own any external references (task records)."""
        _require_id(layout_id)
        _require_id(new_id)
        if new_id == layout_id:
            return "unchanged"
        ...
        if self.exists(new_id):
            raise LayoutError(f"layout '{new_id}' already exists")
        for view in VIEWS:
            src = self.snapshot_path(layout_id, view)
            if os.path.isfile(src):
                os.replace(src, self.snapshot_path(new_id, view))
            elif view == VIEWS[0]:
                legacy = os.path.join(self.root, f"{layout_id}.jpg")
                if os.path.isfile(legacy):
                    os.replace(legacy, self.snapshot_path(new_id, view))
        with open(self.json_path(layout_id), "r", encoding="utf-8") as f:
            raw = json.load(f)
        if isinstance(raw, dict):
            raw["id"] = new_id
            raw["updated_at"] = _iso_now()
        _atomic_write(self.json_path(new_id),
                      json.dumps(raw, indent=2, ensure_ascii=False).encode("utf-8"))
        os.unlink(self.json_path(layout_id))
        return f"renamed layout '{layout_id}' -> '{new_id}'"
