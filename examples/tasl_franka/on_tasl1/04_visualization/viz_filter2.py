...
OUT="/tmp/filter_videos2"; os.makedirs(OUT,exist_ok=True)
FPS=15.0; S=2
TH_POS, TH_ROT, TH_GRIP = 0.198, 0.044, 0.002
MIN_IDLE, MIN_NON, DROP_LAST, MIN_FINAL, GK = 15, 26, 10, 16, 8
...
def seg_rules(idle):
    ...
    while i<n:
        if idle[i]:
            ...
            if j-i>=MIN_IDLE: keep[i:j]=False      # 连续 >=15 帧 idle 整段删
            i=j
        else: i+=1
...
for tix in sorted(by):
    ...
    prot=dilate(dg>TH_GRIP,GK)                       # 抓取动作前后 +-8 帧保护
    raw=(dp<TH_POS)&(ang<TH_ROT)&(dg<TH_GRIP)&(~prot)  # 逐帧判据命中(含夹爪)
    ...
    for i in range(n):
        ...
        cv2.putText(hdr,"ep%06d  frame %d/%d   dpos %.3f mm  drot %.3f deg  dgrip %.4f%s"%(ep,i+1,n,dp[i],ang[i],dg[i]," [GRASP]" if prot[i] else ""),
        ...
        cv2.putText(hdr,"red=drop   [GRASP]=gripper-protected",(W-330,68),cv2.FONT_HERSHEY_SIMPLEX,0.42,(150,150,150),1,cv2.LINE_AA)
